...
<div class="container mt-4">
    <div class="viewProductSearching mb-2">
        <form action="${pageContext.request.contextPath}/manager/products/search" method="get" class="w-100  mr-3">
            <div class="input-group">
                <input value="${requestScope.search}" type="text" class="form-control" id="search"
                       placeholder="<fmt:message key='products.searchPlaceholder' bundle='${lang}' />"
                       name="id"/>
                <div class="input-group-append">
                    <button class="btn btn-outline-secondary" type="submit"><fmt:message key="products.searchButton"
                                                                                         bundle="${lang}"/></button>
                </div>
            </div>
        </form>
    ...
    <c:choose>
        <c:when test="${error!=null}">
            <p style="color: red"><fmt:message key='${error}' bundle='${lang}'/></p>
        </c:when>
    </c:choose>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="button" class="btn btn-primary"
                                   value="<fmt:message key='products.table.infoButton' bundle='${lang}' />"
                                   onclick="window.location.href='${pageContext.request.contextPath}/products/${product.getId()}'">
                        </td>
                        <td>
                            <input type="button" class="btn btn-success"
                                   value="<fmt:message key='products.table.editButton' bundle='${lang}' />"
                                   onclick="window.location.href='${pageContext.request.contextPath}/manager/products/edit/${product.getId()}'">
                        </td>
                        <td>
                            <form action="${pageContext.request.contextPath}/manager/products/delete/${product.getId()}"
                                  method="post">
                                <input type="submit"
                                       value="<fmt:message key='products.table.deleteButton' bundle='${lang}' />"
