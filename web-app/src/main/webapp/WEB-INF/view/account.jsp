...
<div class="container-fluid mt-2">
    <div class="row">
        ...
        <div class="col-lg-10 col-mg-12">
            ...
            <input type="button" class="btn btn-danger mt-1 mb-2"
                   value="<fmt:message key='account.logout' bundle='${lang}' />"
                ...
                <c:otherwise>
                    ...
                    <c:forEach var="order" items="${user.orders}">
                        <div class="mt-4">
                            ...
                            <strong><fmt:message key="account.totalPrice"
                                                 bundle="${lang}"/> ${order.totalPrice}</strong><br>
                            <p><fmt:message key="account.orderTime" bundle="${lang}"/>: <fmt:formatDate
                                    value="${order.createdAtAsDate}"
                                    pattern="yyyy-MM-dd HH:mm"/></p>
                            <p><fmt:message key="account.deliveryAddress"
                                            bundle="${lang}"/>: ${order.address.region}, ${order.address.town}, ${order.address.exactAddress},${order.address.postalCode}</p>
                            <div class="row">
                                        ...
                                        <div class="card mt-2 mb-2" style="width: 17.5rem;height: 24rem">
                                            <a href="${pageContext.request.contextPath}/products/${orderItem.product.id}">
                                                <c:choose>
                                                    ...
                                                    <c:otherwise>
                                                        <img src="${pageContext.request.contextPath}/images/${orderItem.product.imageList[0].id}"
                                                             class="card-img-top"/>
                                                    </c:otherwise>
                                                </c:choose>
                                            </a>
                                            <div class="card-body">
                                                ...
                                                <fmt:message key="account.quantity"
                                                             bundle="${lang}"/> ${orderItem.amount}
                                            </div>
                                        </div>
                                    </div>
                                </c:forEach>
