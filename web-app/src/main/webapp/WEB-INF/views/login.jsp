            ...
            <div class="card" style="width: 18rem;">
                ...
                <div class="card-body">
                    <form action="${pageContext.request.contextPath}/login" method="post">
                        ...
                        <div class="mb-3">
                            <label for="login" class="form-label"><fmt:message key="login.email" bundle="${lang}"/></label>
                            <input type="email" value="${requestScope.email}" class="form-control" id="login" name="email">
                        </div>
                        <div class="mb-3">
                            <label for="password" class="form-label"><fmt:message key="login.password" bundle="${lang}"/></label>
                            <input type="password" value="${requestScope.password}" class="form-control" id="password" name="password">
                        </div>
                        <button type="submit" class="btn btn-primary"><fmt:message key="login.submit" bundle="${lang}"/></button>
                    </form>
                </div>
            </div>
            <div id="add-info">
                <h5 class="mt-4"><fmt:message key="login.noAccount" bundle="${lang}"/></h5>
                <a href="${pageContext.request.contextPath}/registration" class="btn btn-primary"><fmt:message key="login.register" bundle="${lang}"/></a>
